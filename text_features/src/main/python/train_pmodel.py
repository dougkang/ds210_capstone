...
def post_pmodel(X, y, m, k, path, **kwargs):
  '''
  ...
  y_pred = np.ndarray(shape=(y.shape[0], int(k)), dtype=object) 
  for i,ps in enumerate(m.predict_proba(X)):
    top = sorted(enumerate(ps), key=lambda x: x[1], reverse=True)[:int(k)]
    y_pred[i, :] = [ vocab[x[0]] for x in top ]

  y_truth = np.ndarray(shape=(y.shape[0], 1), dtype=object) 
  ...
  metrics['precision'] = (float(matched) / retrieved)
  metrics['recall'] = (float(matched) / relevant)
